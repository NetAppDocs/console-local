...
summary: Install a CA-signed certificate for user access to the web-based console when using NetApp Console (local deployment) in restricted mode.
---

= Install a CA-signed certificate for NetApp Console (local deployment) access
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
When you use the NetApp Console in restricted mode or private mode, the user interface is accessible from the Console agent virtual machine that's deployed in your cloud region or on-premises. By default, the Console uses a self-signed SSL certificate to provide secure HTTPS access to the web-based console running on the Console agent. 
