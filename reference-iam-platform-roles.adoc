...
keywords: predefined roles, iam roles, organization admin, folder or fleet admin, classification viewer, snapcenter admin, role, iam role, admin, iam, identity and access management, bluexp
summary: Assign platform roles to users to grant permissions to manage the NetApp Console, assign roles, add users, and manage federations.
---

= NetApp Console (local) platform access roles
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Assign platform roles to users to grant permissions to manage the NetApp Console (local), assign roles, add users, create fleets, and manage user authentication.
...
The *Organization admin* in XYZ Corporation's Console (local) creates an initial organization and separate folders for each region. The *Folder or fleet admin* for each region organizes fleets (with associated resources) within the region's folder.
...
The following tables indicate the actions that each Console (local) platform role can perform.
...
| Create, modify or delete systems from the Console (local) (add or discover systems) |	Yes | Yes 
