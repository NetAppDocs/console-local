...
= Ports for the on-premises Console agent in NetApp Console local deployment
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
In NetApp Console local deployment, the Console agent uses _inbound_ ports when installed manually on an on-premises Linux host. Refer to these ports for planning purposes.
