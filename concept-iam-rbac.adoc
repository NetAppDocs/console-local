...
To choose access for a member, match the role category to the member's responsibilities, then assign the role at the scope (organization, folder, or fleet) that matches how broadly the member should have that access. link:concept-folders-fleets-overview.html#organization-design-examples[Learn about how different organizations structure roles and scope in NetApp Console local deployment].
