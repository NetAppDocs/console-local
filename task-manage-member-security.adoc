...
Instruct a local user to reset their password from the Console (local deployment) login page by selecting *Forgot password?*.
...
NOTE: Multi-factor authentication is only available for local users. Directory-authenticated users cannot enable MFA through Console (local deployment).
