...
* CPU: 16 cores or 16 vCPUs (reserved)
* RAM: 64 GB (reserved)
...
[NOTE]
====
* CPU and memory are reserved on the ESXi host when the VM powers on, not just allocated. Ensure the target host has at least 16 CPU cores and 64 GB available as reserved capacity. Existing VM reservations on the host reduce available capacity.
* The datastore must meet minimum disk write performance: fsync p99 write latency ≤ 20 ms and 4K random write IOPS ≥ 200. Latency above 50 ms or IOPS below 30 causes deployment failure.
* IPv6 is not supported. A static IPv4 address is required.
