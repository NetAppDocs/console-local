...
[NOTE]
====
* CPU and memory are reserved on the ESXi host when the VM powers on, not just allocated. Ensure the target host has at least 16 CPU cores and 64 GB available as unreserved capacity. Existing VM reservations on the host reduce available capacity.
