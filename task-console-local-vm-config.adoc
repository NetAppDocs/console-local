...
summary: You can make changes to your existing VCenter or ESXi host after you deploy Console (local deployment). For example, you can increase the CPU or RAM of the VM instance that hosts Console (local deployment). 
---
...
[.lead]
You can make changes to your existing VCenter or ESXi host after you deploy the Console (local deployment). For example, you can increase the CPU or RAM of the VM instance that hosts the Console (local deployment). 
...
. Select the VM instance that hosts the Console (local deployment).
...
. Open the VSphere client and log in to your VCenter.
. Select the VM instance that hosts Console (local deployment).
. Select *Launch Web Console*.
. Log in to the VM instance using the user name and password that you specified when you created the VM instance. The username is `maint` and the password is the one that you specified when you created the VM instance.
. Enter `1 to view the `System Configuration` menu.
...
+ For example, you can verify that all Console (local deployment) services are running. link:task-troubleshoot-console-local.html#check-console-local-status[Check the Console (local deployment) status^].
...
. Use an SSH tool such as Putty to connect to the Console (local deployment) VM using the user name `diag` and the one-time password that you configured.
