---
sidebar: sidebar
permalink: task-partnerships-resource-access.html
keywords: role-based access, partnership access, partner resources, resource access, grant acces, provide partner access, partnership organization
summary: You can grant access to partnership users by assigning them specific roles for folders and fleets within your organization.
---

= Provide resource access to partnership users
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
:imagesdir: ./media/

[.lead]
You can grant access to partnership users by assigning them specific roles for folders and fleets within your organization.

.Required roles
Partnership admin. link:reference-iam-predefined-roles.html[Learn more about access roles.]

A partner organization must first add members to the partnership before you can assign them roles for resources in your organization. link:task-partnerships-assign-users.html[Learn how to add members to a partnership.]

== Understand roles for partnership users

You can manage roles for members of partner organizations in the same way that you do for your own. However, not all roles are available to partnership users. In particular, you can't grant partner users a role that allows software updates. Updating ONTAP software generally requires direct network access.

You can assign following roles to partner users:

* link:reference-iam-platform-roles.html#organization-admin-roles[Organization admin]
* link:reference-iam-platform-roles.html#organization-admin-roles[Folder or fleet admin]
* link:reference-iam-platform-roles.html#federation-roles[Federation admin]
* link:reference-iam-platform-roles.html#federation-roles[Federation viewer]
* link:reference-iam-backup-rec-roles.html[Backup and recovery admin]
* link:reference-iam-backup-rec-roles.html[Backup viewer]
* link:reference-iam-backup-rec-roles.html[Restore admin]
* link:reference-iam-backup-rec-roles.html[Clone admin]   
* link:reference-iam-disaster-rec-roles.html[Disaster recovery admin]
* link:reference-iam-disaster-rec-roles.html[Disaster recovery failover admin]
* link:reference-iam-disaster-rec-roles.html[Disaster recovery application admin]
* link:reference-iam-disaster-rec-roles.html[Disaster recovery viewer]
* link:reference-iam-storage-roles.html[Operations support analyst]
* link:reference-iam-predefined-roles.html[Classification viewer]
* link:reference-iam-keystone-roles.html[Keystone admin]
* link:reference-iam-keystone-roles.html[Keystone viewer]

link:reference-iam-predefined-roles.html[Learn more about predefined roles]



== Add a role to a partner user
You provide access to your organization's resources by adding a role to a member. When you assign a role, you specify one resource and one role. You can assign more than one role to a user.

For example, if you had two fleets and wanted the same user to have the role of Backup and recovery admin for both, you would need to provide the role to the user for each fleet. Similarly if you wanted to provide a user with two different roles for the same fleet, you would need to assign each role separately.


.Steps

. Select *Administration > Identity and access*.

. Select *Partnerships*.

. Select the *Partnership initiated* tab.

. Select the actions menu image:icon-action.png[an icon with three vertical dots] next to the established partnership that you want view and select *View details*.
+ 
The *Member* list displays the members that the partner organization has added to the partnership.

include::_include/tasks-role.adoc[tag=add-role]


== Change or remove a role from a partner user
You can change or remove a role that you have assigned to a member of a partner organization.

.Steps
. Select *Administration > Identity and access*.
. Select *Partnerships*.
. Select the *Partnership initiated* tab.
. Select the actions menu image:icon-action.png[an icon with three vertical dots] next to the established partnership that you want view and select *View details*.
+ 
The *Member* list displays the members that the partner organization has added to the partnership.

include::_include/tasks-role.adoc[tag=remove-role]






