...
keywords: predefined roles, iam roles, organization admin, folder or fleet admin, classification viewer, snapcenter admin, role, iam role, admin, iam, identity and access management, access roles, rbac, bluexp
...
= NetApp Console (local) access roles
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Identity and access management (IAM) in NetApp Console (local) provides predefined roles that you can assign to the members of your organization across different levels of your resource hierarchy. Before you assign these roles, you should understand the permissions that each role includes. Roles fall into the following categories: platform, application, and data service.


[[platform-roles]]
== Platform roles
Platform roles grant NetApp Console (local) administration permissions, including role assignment and user management. The Console (local) has several platform roles. 
...
| link:reference-iam-platform-roles.html#federation-roles[Federation admin]|	Allows a user to create and manage federations (including directory service integrations) with the Console, which enables single-sign on (SSO). 
| link:reference-iam-platform-roles.html#federation-roles[Federation viewer]|	Allows a user to view existing federations and directory service integrations with the Console. Cannot create or manage federations.
