...
If you want to confirm who can access a specific fleet, review the members assigned to that fleet instead. link:task-users-fleets-assign.html[Manage fleet access assignments].
...
A role that you assign at the folder level applies to all child fleets and folders, while a role that you assign at the fleet level applies only to that fleet's resources. If a member inherits access from the organization or a folder, change that assignment at the higher scope where you granted it. link:concept-iam-rbac.html#role-inheritance[Learn more about how role inheritance works].
