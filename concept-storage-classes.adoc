...
After provisioning, Console (local deployment) continuously evaluates each workload against its storage class standards and alerts you if the workload drifts out of compliance.
...
5. *Monitor workloads for drift* - Console (local deployment) continuously evaluates each workload against the standards encoded in its storage class. If a configuration change takes the workload outside those standards, Console (local deployment) detects the drift and raises an alert.
6. *Remediate drift to restore compliance* - When drift is detected, you can follow guided steps to correct the issue manually, let Console (local deployment) resolve common drift conditions automatically, or disassociate a workload from its storage class if you need to change its settings.
...
* To monitor and remediate drift, see link:task-fleet-health-monitor.html[monitor fleet health], link:task-remediation-guided.html[guided remediation], and link:task-remediation-automated.html[automated remediation].
