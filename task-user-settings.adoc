...
= Manage your NetApp Console (local) user settings
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
You can modify your Console (local) profile including change your password, enable multi-factor authentication (MFA), and see who your Console administrator is.


== Change your display name

You can change your Console (local) display name, which identifies you to other users. You cannot change your username or email address.

.Steps

. Select the profile icon in the upper right corner of the Console (local) to view the User settings panel.
...
NOTE: You can't configure MFA for your account through the Console (local) if your account uses Active Directory or another integrated directory service for authentication.
...
After you set up MFA, the Console (local) prompts you to enter a one-time code from your authentication app each time you log in.

== Regenerate your MFA recovery code
You can only use a recovery code once. If you lose yours, create a new one.

.Steps
. Select the profile icon in the upper right corner of the the Console (local) to view the User settings panel.
...
When you're finished, either log out of the Console (local) or move the slider back to return to read-only mode. The system removes your elevated permissions when your session expires.
...
You can set the Console (local) to display in dark mode.
