...
* If the storage efficiency ratio is dropping, review whether the workload's data type or inline efficiency settings are reducing savings. Use link:reference-workload-analyzer-metrics.html[Learn about Workload Analyzer metrics in NetApp Console local deployment] for detailed descriptions of capacity, snapshot, and efficiency metrics.
* If the volume has a capacity-related alert that Console local deployment can resolve, the alert might offer a Fix-It option. See link:task-remediation-guided.html[Remediate with guided steps].
