...
summary: Discover on-prem ONTAP clusters from NetApp Console (local deployment) so that you can start managing volumes and performing advanced management using ONTAP System Manager, which is available from the Console.
---

= Discover on-premises ONTAP clusters in NetApp Console (local deployment)
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
Storage admin or System health specialist. link:reference-iam-predefined-roles.html[Learn about access roles].
