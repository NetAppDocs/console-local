...
* If you configure an Anthropic endpoint, Console local deployment sends prompts and context to the configured Anthropic endpoint.
...
Control Console assistant costs by choosing an appropriate LLM provider and model, setting usage limits and alerts with your provider, and monitoring token usage. Only activities that send requests to the LLM consume provider tokens; backend operations do not.
