...
= Google Cloud permissions for the NetApp Console agent
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
| config.deployments.get   |No | Yes| Yes
...
| compute.disks.create .8+|To create and manage disks on Cloud Volumes ONTAP systems. | Yes | Yes | No
...
| compute.disks.update | No | Yes | No
| compute.disks.use | No | Yes | No
| compute.firewalls.create .4+| To create firewall rules for Cloud Volumes ONTAP. 
| Yes | No | No
...
| compute.forwardingRules.update | Update existing forwarding rules for traffic management. | No | Yes | No
...
| compute.fleets.get | To support multi-fleets. | Yes | No | No
...
| resourcemanager.fleets.get | To support multi-fleets. | Yes | Yes | No
...
| cloudbuild.workerpools.get| To access worker pool information during private mode deployment and conversion of Cloud Volumes ONTAP systems using Infrastructure Manager. | Yes | Yes | Yes
| cloudbuild.workerpools.list | To list worker pool information during private mode deployment of Cloud Volumes ONTAP systems using Infrastructure Manager. | Yes | No | No
...
=== 13 April 2026
The `compute.disks.update` permission is added to update IOPS and throughput parameters on a Hyperdisk Balanced aggregate or disk in Cloud Volumes ONTAP systems.

=== 26 February 2026

The `cloudbuild.workerpools.get` and `cloudbuild.workerpools.list` permissions are added for supporting Infrastructure Manager in private mode deployments of Cloud Volumes ONTAP in Google Cloud.

=== 09 February 2026

The `compute.forwardingRules.update` permission is added for supporting Infrastructure Manager in Cloud Volumes ONTAP deployments in Google Cloud.
