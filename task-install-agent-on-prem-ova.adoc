...
[.lead]
If you are a VMWare user, you can use an OVA to install a Console agent in your VCenter. THe OVA download or URL is available through the NetApp Console Local.
...
NOTE: Install the agent in a vCenter environment rather than directly on an ESXi host. 
...
NetApp Console Local needs access to the networks where your ONTAP clusters and other resources are located. This allows the Console Local to discover those resources, monitor their health, and perform actions on them. If your agent doesn't have access to a target network, the Console Local won't be able to manage the resources on that network.
...
NOTE: If the deployment fails, you can access the validation report and logs from the agent host. link:task-troubleshoot-agent.html#troubleshoot-installation[Learn how to troubleshoot installation issues.]
